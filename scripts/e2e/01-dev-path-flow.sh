...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/cursor-e2e-dev-path.XXXXXX")"
project_dir="${workdir}/demo-backend"
...
# demo-backend
Brownfield sample project for cursor-engineering-bootstrap E2E.
EOF

echo "[e2e] step 1/7: initialize sample project (dry-run + bundle + spec-kit init)"
...
assert_dir "${project_dir}/_cursor_init/patch_bundle/backend"
assert_dir "${project_dir}/.specify"
assert_contains "${project_dir}/_cursor_init/report.md" "spec_kit_init"

echo "[e2e] step 2/7: apply patch bundle to sample project"
rsync -a "${project_dir}/_cursor_init/patch_bundle/backend/" "${project_dir}/"
assert_file "${project_dir}/PR_TEMPLATE.md"
assert_file "${project_dir}/.cursor/hooks/gates/pr-dev-path-check.sh"
...
expect_success "main path stage1 block" \
...
expect_success "main path stage2 block" \
...
expect_success "fast path stage1 block" \
...
expect_success "fast path stage2 block" \
...
expect_failure "stage1 block rejects none selected" \
...
expect_failure "stage1 block rejects both selected" \
...
expect_failure "stage2 block rejects fast path without implement evidence" \
...
expect_success "stage2 warn allows missing evidence with warning" \
...
echo "[e2e] PASS: backend dev-path flow end-to-end"
